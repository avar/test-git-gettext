...
(cd t && for test in ./t02*sh; do bash ./$test; done) | sed 's/^/    /' >> $LOG 2>&1
(cd t && for test in ./t02*sh; do bash ./$test -d -v; done) | sed 's/^/    /' >> "$LOG-raw" 2>&1
...
(cd t && for test in ./t02*sh; do TEST_GIT_I18N_EXHAUSTIVE=1 bash ./$test; done) | sed 's/^/    /' >> $LOG 2>&1
(cd t && for test in ./t02*sh; do TEST_GIT_I18N_EXHAUSTIVE=1 bash ./$test -d -v; done) | sed 's/^/    /' >> "$LOG-raw" 2>&1
